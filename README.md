...
## v6.2 달빛공원 API 방식
달빛공원은 Selenium/Playwright로 달력과 날짜를 순서대로 클릭하는 대신, 사이트 내부 Convex 클라이언트의 공개 쿼리를 우선 사용합니다. 브라우저는 앱과 세션을 로드하는 용도로만 사용하며, 시설 ID를 자동으로 찾지 못한 코트에 한해서 기존 화면 탐색 방식으로 보완합니다.
