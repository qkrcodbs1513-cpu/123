...
연수문화공원과 달빛공원 테니스장 빈자리를 감시해 텔레그램으로 알려주는 Railway용 봇입니다.

## 주요 명령어
- `/settings`: 공원 ON/OFF, 코트, 재질, 시간 설정
- `/status`: 마지막 검사, 사이트별 조건 일치, 최근 신규 알림, 누적 통계 확인
- `/help`: 명령어 안내

## 달빛공원
- 5~14번 코트 facilityId 10개 기본 포함
- API 우선 조회, 실패할 때만 DOM 자동 복구
- 5~8번 하드코트 / 9~14번 인조잔디
- 코트별·재질별 필터 지원

## 로그
기본은 API 결과와 검사 요약만 출력합니다. 상세 진단이 필요할 때 Railway Variables에 `DALBIT_DEBUG=1`을 추가하세요.
