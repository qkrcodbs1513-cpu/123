...
def get_available_slots():
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for r in rows:
                        ...
                        if ("신청" in text or "예약" in text or "가능" in text) and ("시" in text or ":" in text or "~" in text):
    ...
    try:
        ...
        res_m = session.get(url_m, headers=headers, timeout=10)
        if res_m.status_code == 200:
            soup_m = BeautifulSoup(res_m.text, "html.parser")
            elements = soup_m.find_all(string=re.compile(r"예약가능|신청"))
    ...
    try:
        ...
        res_n = session.get(url_n, headers=headers, timeout=10)
        if res_n.status_code == 200:
            soup_n = BeautifulSoup(res_n.text, "html.parser")
            elements = soup_n.find_all(string=re.compile(r"신청가능|예약가능"))
            for elem in elements:
                parent = elem.parent.parent if elem.parent else elem
                text = parent.get_text(" ", strip=True)
    ...
    print(f"📊 [크롤링 완료] 총 슬롯 수: {len(unique_slots)}개")
    return unique_slots
